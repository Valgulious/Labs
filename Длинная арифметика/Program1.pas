 ...
 {Разбиванеи разряда из целой части числа на сотни, десятки и еденицы (для сложения и вычитания)}
  ...
  function ReadRealNumber (f: text; var x: RealNumber): boolean;
    ...
    begin
      ...
      if (d1 > 150) or (d2 > 150) then // Для случая, когда в целой или в целой части больше 150 цифр
        b := false;
  ...
  {Печать числа в файл}  
  function WriteRealNumber (var t:text; x: RealNumber):boolean;
    ...
    {Добавление недостающих нулей к разряду}
    procedure convert (var s: string; x: RealNumber; i,y:byte);
      
      begin
        if (y = 1) then // Если y = 1 работает с целой частью, иначе с дробной частью
          begin
            s := IntToStr(x.IntPart[i]);
            if (length(s) = 1) then
              s := '00' + s
            else
              if (length(s) = 2) then
    ...
    begin
      b := true;
      if (x.IntPart[0] <> 0) or (x.RealPart[0] <> 0) then
        begin
          if (x.IntPart[0] > 1) then
            begin
              write(t,x.IntPart[x.IntPart[0]]);
              for i := x.IntPart[0] - 1 downto 1 do
                begin
                  convert(s,x,i,1);
                  write(t,s);
                end;
            ...
            begin
              s := IntToStr(x.IntPart[1]);
              write(t,s);
            end;
          write(t,'.');
          if (x.RealPart[0] > 1) then
            begin
              for i := 1 to x.RealPart[0] - 1 do
                begin
                  s := '';
                  convert(s,x,i,2);
                  write(t,s);
              ...
              begin
                if (x.RealPart[1] <> 0) then
                  write(t,last_convert(x))
                else
                  begin
                    s := IntToStr(x.RealPart[1]);
                    write(t,s);
                  end;
              end;
          writeln(t);
  ...
  {Сложение чисел}
  function SumRealNumbers (x,y: RealNumber; var SumNumbers: RealNumber):boolean;
  ...
  function SumIntParts (x,y:RealNumber; var SubNumbers:RealNumber; d: boolean): boolean;
      ...
      begin
        if (x.IntPart[0] < y.IntPart[0]) then
          begin
            k := y.IntPart[0];
            SumNumbers.IntPart[0] := y.IntPart[0];
          end
        else
          begin
            k := x.IntPart[0];
            SumNumbers.IntPart[0] := x.IntPart[0];
          end;
        for i := 1 to k do
          begin
            p := false;
            ...
            if (d) then
              ...
              d:=sum(a,a1,sa,p);
            SumNumbers.IntPart[i] := 100*sa + 10*sb + sc;
          end;
        SumIntParts := d;
      end;
    
  begin
    d := SumRealParts (RealNumber1,RealNumber2,SumNumbers);
    if (SumIntParts(RealNumber1,RealNumber2,SumNumbers,d)) then // Делалось для переволнения
      begin
        SumNumbers.IntPart[0] := SumNumbers.IntPart[0] + 1;
        SumNumbers.IntPart[SumNumbers.IntPart[0]] := 1;
        if (SumNumbers.IntPart[0] > n) then
          SumRealNumbers := true
        else
          SumRealNumbers := false;
      end;
