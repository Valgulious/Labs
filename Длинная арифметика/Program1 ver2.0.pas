 ...
 {Разбиванеи разряда из целой части числа на сотни, десятки и еденицы (для сложения и вычитания)}
  ...
  function SumRealNumbers (x,y: RealNumber; var SumNumbers: RealNumber):boolean;
    ...
    begin
      ...
      f := x.RealPart[k];
      s := y.RealPart[k];
      z := f div 100 + s div 100 + d;
      d := z div 8;
      SumNumbers.RealPart[k] := SumNumbers.RealPart[k] + (z mod 8)*r;
      r := r*10;
      f := f mod 100;
      s := s mod 100;
      z := f div 10 + s div 10 + d;
      d := z div 8;
      SumNumbers.RealPart[k] := SumNumbers.RealPart[k] + (z mod 8)*r;
      r := r*10;
      f := f mod 10; 
        ...
        begin
          ...
          for j := 1 to 3 do
            begin
              ...
              d := z div 8;
              SumNumbers.RealPart[i] := SumNumbers.RealPart[i] + (z mod 8)*r;
              r := r*10;
              f := f div 10; 
              s := s div 10;
            end;
